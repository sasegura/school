  ...
  return <>
    <div className="col-lg-8 col-sm-12">
      <select  className={"input-form" + errorClass(errors.course)}  {...register("course", {required: true})}>
        ...
        {courseList.map((cour) => 
          <option key={cour.id} value={cour?.id} selected={cour?.id === course?.id} id={cour.name}>
            {cour?.name}
          </option>)}
      </select>
    </div>
    <div className="col-lg-4 col-sm-12">
      <select className={"input-form" + errorClass(errors.edition)} {...register("edition", {required: true})}>
        <option value="" disabled > Edición</option>
        {editionList.map((edit) => 
          <option value={edit?.id} key={edit?.id} selected={edit?.id === edition?.id} id={edit.name}>
            {edit?.name}
          </option>)}
      </select>
    </div>
